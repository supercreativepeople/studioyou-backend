...
- Anthropic pipeline: Partner Network investment lane via partner manager (in motion); Anthology Fund is primary, submit after S2 with demo video. Lee is a Claude partner and has applied to several partnership programs offering financing and credits.
- **Applications (recorded 2026-08-09):** all Claude program applications were filed with **Frisson Digital, Inc. as the applicant**. Lee's position, reviewed and decided: the specific Claude *account* used is immaterial to eligibility, which turns on the product being built on Anthropic and on corporate ownership meeting the requirements. Agreed on eligibility. One narrow mechanical caveat kept on the record: **granted credits land in whichever Console org is linked to the application, and credits are non-transferable between orgs.** So the account does determine where any award shows up, even though it does not determine whether Frisson qualifies. Worth confirming the applications point at the org intended to carry production traffic, particularly if any were filed under a personal-email Console account rather than a Frisson one.
- **Startup Program: reopened by the Frisson restructure (revisited 2026-08-09).** The note that Startup Program credits were "DEAD" was correct *for SCP Inc.*, whose 2019 incorporation fails the four-year window. **Frisson Digital, Inc. is a new entity and passes that test.** Per the published eligibility (checked 2026-08-09, claude.com/programs/startups): the company must have (1) received equity funding from an institutional investor, (2) been founded within the last four years, (3) not previously received Anthropic startup credits. Frisson clears (2) and (3). The live blocker is now **(1), institutional equity funding**, which is a different and more addressable problem than a fixed incorporation date. Note also that startups **referred or nominated by an Anthropic partner VC or accelerator** can scale to $100k packages, so an accelerator acceptance would plausibly satisfy the funding condition and the nomination route at once. Credits run roughly 12 months and are non-dilutive. Worth re-checking current terms directly before applying, as program terms change.
- **Entity (SUPERSEDED — corrected 2026-08-09):** the per-product newco plan below did not happen. **Frisson Digital, Inc. was formed as a parent company owning both StudioYou and SCREENBot**, instead of creating individual newcos per product. Rationale: a single parent holding both products aligns with incubator programs, Anthropic's programs, and other fundraising/incubator fund opportunities in a way that scattered per-product entities do not. **All billing instances are to be established through Frisson Digital, Inc.** Confirmed by Lee 2026-08-09: today, **not one** does. Every paid resource is personally funded by Lee on personal cards. **SCP Inc. owns nothing** and has no IP assigned to it. **StudioYou and SCREENBot each have executed IP assignment documentation to Frisson**; Lee's other products remain his personally, unassigned. StudioYou and SCREENBot are **independent products** under a common parent, a corporate fact and not a shared-codebase fact. See `SERVICES.md` for the per-account picture. Note the diligence gap: Frisson owns the IP while Lee's personal cards fund the infrastructure it runs on, which is counsel-and-accountant territory, not a dev task.
  - *Superseded plan, kept for history:* SCP formalizes as venture studio; StudioYou Inc. (Delaware C-corp) formed before Anthology application (late July, counsel engaged by Lee); all IP currently vests in Lee personally (no assignments exist anywhere), assigned directly at formation; SCP retained stake teens-to-low-20s, Lee holds founder shares personally.
  - *Open:* the IP-assignment question needs re-answering against the Frisson structure. The superseded plan had IP assigned at StudioYou Inc. formation, which no longer exists as a step.
- Full record: [Session Log July 6, 2026](https://www.notion.so/396b963047e581cc866ec819e878d456)

---
**Last Updated:** August 9, 2026 — platform/partner/subscription tracker build-out; corporate structure correction (Frisson Digital, Inc.); live-stack audit found 6 untracked paid services.

> **ALPHA IS BLOCKED ON TWO UNPAID ACCOUNTS (2026-08-09).** LiveKit has a **$50 balance due, unpaid**. Runway's **credits are exhausted** and the account needs a top-up. Both live in `studioyou-fy-agent`. Until both are settled, **no live FY session can run**, which means item 1 below (the live IDEATE retest, the gate on alpha close) cannot be attempted. Neither is a technical failure; the avatar feature itself is confirmed working. See `SERVICES.md` in this repo and in `studioyou-fy-agent` for the full picture.

**Next Session:** Priority order:
0. **Settle LiveKit ($50) and top up Runway.** Blocks everything below it. Then verify Fal.ai, Deepgram, and Cartesia standing, any of which could be a further hidden blocker (all three were invisible to the tracker until 2026-08-09).
...
**main.py — Session AE (current HEAD — commit `bd6b306`; was 46d020d through AD)**
...
**studioyou-fy-agent repo — Session W, AA, AE**
