...
- **2026-09-06:** FY quality evaluation loop complete. (1) FY Model Routing Spec written to StudioYou project (`claude/FY_ModelRouting_Spec_2026-09-06.md`) — locks Reference-Frame-First chaining (Firefly → LTX/Seedance), prompt post-mortem loop, vault-as-creative-brief. (2) `/api/quality_judge` endpoint built in main.py — Claude vision-as-judge, fails open on all error paths, video pass-through pending frame extraction. Commit `8e806ce`. (3) Vault `creative_brief` wired into studio.html — `fyQualityJudge()` extended to accept brief, `runGeneration()` computes brief from `_cdState.vaultAssets`. Commit `8faa876`. Backend HEAD: `8e806ce`. Frontend HEAD: `8faa876`.
- **2026-09-05:** CLAUDE.md corrections only — no code changes. (1) Live State updated: HEAD now `d4e2f19`, Cloud Run revision now `00463-pj7` (RUNWAYML_API_SECRET, CARTESIA_API_KEY, RUNWAY_IMAGE_CONFIG_ID=future-you-router added to Cloud Run env). (2) Previously `00460-jdg`. (2) Subscription tier pricing corrected against Revenue & Pricing Model v2 (authoritative): Independent is $199/mo monthly / $129/mo annual + $149 onboarding; Operator is $149/mo monthly / $99/mo annual; Founding tier added (first 500, annual only); "Player" tier removed (never existed in the revenue model). Prior CLAUDE.md had wrong prices and a phantom tier.
...
|---|---|
| Backend HEAD | commit `8e806ce` — feat: /api/quality_judge vision-as-judge endpoint |
| Cloud Run revision | 00463-pj7 (deployed 2026-09-05T04:35:39Z) — new revision deploying via Cloud Build after 8e806ce push |
...
| studio.html | Current HEAD: `8faa876` — vault creative_brief wired into fyQualityJudge + runGeneration. GitHub auto-deploy → studioyou.app. |
