...
| Supabase (shared) | Storage/Database | `fy_vault_entries` table (creator answer capture) | Lee (personal) | Free tier (auto-pauses after 7 days idle) | $0 | no | project ref `rubwhfjwqonqhfbkhren`, org `yepuslepymluiqycrkfd` | app.supabase.com | unconfirmed (region us-east-2, Postgres 17) | n/a | SUPABASE_URL / SUPABASE_KEY in .env and Cloud Run env | **Active — restored by Lee 2026-08-16, see dated section below** | 2026-08-16 |
...
- [x] ~~Decide whether to restore the Supabase project~~ — resolved 2026-08-16. Lee restored it manually; confirmed `ACTIVE_HEALTHY` via Supabase MCP. Cause was free-tier 7-day idle auto-pause, will recur without traffic — see dated section below.
- [x] ~~Resolve the frontend-workflow discrepancy~~ — resolved 2026-08-16. Lee confirmed the manual "Lee provides → Claude modifies → presents" flow predates the current dev-session-protocol and no longer applies. CLAUDE.md's Locked Decisions and Repos & Access table rewritten to match: frontend now goes through the same git workflow as backend/agent (Desktop Commander, commit, push). Committed `649900e`.
...
## Supabase confirmed INACTIVE (paused), 2026-08-16 — RESOLVED same day, see addendum below
...
**Resolved same day, 2026-08-16.** Lee confirmed the cause: Supabase free tier auto-pauses after 7 days of inactivity. He unpaused it manually via the Supabase console. Re-checked directly via `mcp__Supabase__get_project`: **status `ACTIVE_HEALTHY`.** No longer an alpha blocker. Root cause is inactivity, not a billing or credential problem, so it will recur if the project goes 7+ days without traffic — worth a standing reminder rather than a one-time fix (e.g. a lightweight scheduled query, or just checking status at the start of any session touching the vault after a gap).

## Frontend repos are git-tracked and being committed to directly — RESOLVED, see addendum below
...
**Resolved same day, 2026-08-16.** Lee confirmed: the manual workflow predates the current dev-session-protocol and had a real problem behind it — files backed up to Lee's local Mac only ever lived there, with no running record of session activity once delivered. That manual workflow no longer applies. CLAUDE.md's Locked Decisions and Repos & Access table rewritten to match reality: frontend work goes through the same git workflow as backend/agent (edit via Desktop Commander, commit, push), never present-and-wait-for-manual-deploy as the default. Committed and pushed `649900e`.
