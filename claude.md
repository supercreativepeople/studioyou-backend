...
**Agent ID:** `CA_TJpDATHwLRkE` — RUNNING, us-east (LiveKit Cloud). **Changed Session AD (fifth recreate)** — dropped `pronunciation_dict_id` (`48b6462`), testing whether sonic-3.5's native pronunciation handles the flagged vocabulary without the dictionary. Documentation (Cartesia/LiveKit and Vapi both) confirms pronunciation dictionaries are officially only supported on sonic-3, not sonic-3.5 — the installed plugin's `_is_sonic_3()` check does a prefix match and lets sonic-3.5 pass client-side, but that doesn't confirm Cartesia's server honors the dict on the newer model. Was `CA_uqgwZctcttb9` (voice swap Nolan → Corey, `630ed21c-2c5c-41cf-9d82-10a7fd668370`, `63a6faa` — Nolan read as too breathy and creepy), before that `CA_bfB2ATWADGC2` (voice swap Jameson → Nolan, `46e6c2a` — Jameson read as unintended accent/ethnicity coding), before that `CA_2P82mL8o4yn5` (pronunciation dict wired — `CARTESIA_PRONUNCIATION_DICT_ID=pdict_EFg3YUoQfZfzyupNVwiFL9`, 27-term FY vocabulary dict; model switched sonic-3 → sonic-3.5; acronym-spacing rule added to FY_OPERATIONAL_RULES; `1c52654`). CARTESIA_TTS_SPEED env var wired but currently a no-op — speed/volume temporarily disabled on sonic-3.5 per Cartesia. Before that `CA_VyEg7PNceKBc` (avatar rotation fix, `dae3ea8`), before that `CA_HLJDfBDvSj8E` (Session AC twice — avatar-billing-stop fix, then the say-verbatim handoff-line fix), before that `CA_gHcpYouSmW6Y`, before that `CA_hJH2hG3UdrR3` (Session AA before that: `CA_eESBzjexFe9C`, deleted and recreated for the Runway swap).
**Update command:** `lk agent update CA_TJpDATHwLRkE` — for config-only changes.
...
**Agent ID:** `CA_TJpDATHwLRkE` — RUNNING, us-east (LiveKit Cloud). Changed Session AD (fifth recreate) — dropped pronunciation_dict_id, testing sonic-3.5 native pronunciation. Was `CA_uqgwZctcttb9` (voice swap Nolan → Corey), before that `CA_bfB2ATWADGC2` (voice swap Jameson → Nolan), before that `CA_2P82mL8o4yn5` (pronunciation dict, sonic-3.5, acronym rule), before that `CA_VyEg7PNceKBc` (avatar rotation fix), before that `CA_HLJDfBDvSj8E` (Session AC twice), before that `CA_gHcpYouSmW6Y`, before that `CA_hJH2hG3UdrR3` (Session AA before that: `CA_eESBzjexFe9C`).
