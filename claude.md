...
**Agent ID:** `CA_uqgwZctcttb9` — RUNNING, us-east (LiveKit Cloud). **Changed Session AD (fourth recreate)** — voice swapped Nolan → Corey (`630ed21c-2c5c-41cf-9d82-10a7fd668370`, "Corey - Supportive Buddy," en-US masculine, `63a6faa`), Nolan read as too breathy and a little creepy on live test. Was `CA_bfB2ATWADGC2` (voice swap Jameson → Nolan, `46e6c2a` — Jameson read as unintended accent/ethnicity coding), before that `CA_2P82mL8o4yn5` (pronunciation dict wired — `CARTESIA_PRONUNCIATION_DICT_ID=pdict_EFg3YUoQfZfzyupNVwiFL9`, 27-term FY vocabulary dict; model switched sonic-3 → sonic-3.5; acronym-spacing rule added to FY_OPERATIONAL_RULES; `1c52654`). CARTESIA_TTS_SPEED env var wired but currently a no-op — speed/volume temporarily disabled on sonic-3.5 per Cartesia. Before that `CA_VyEg7PNceKBc` (avatar rotation fix, `dae3ea8`), before that `CA_HLJDfBDvSj8E` (Session AC twice — avatar-billing-stop fix, then the say-verbatim handoff-line fix), before that `CA_gHcpYouSmW6Y`, before that `CA_hJH2hG3UdrR3` (Session AA before that: `CA_eESBzjexFe9C`, deleted and recreated for the Runway swap).
**Update command:** `lk agent update CA_uqgwZctcttb9` — for config-only changes.
...
**Agent ID:** `CA_uqgwZctcttb9` — RUNNING, us-east (LiveKit Cloud). Changed Session AD (fourth recreate) — voice swap Nolan → Corey. Was `CA_bfB2ATWADGC2` (voice swap Jameson → Nolan), before that `CA_2P82mL8o4yn5` (pronunciation dict, sonic-3.5, acronym rule), before that `CA_VyEg7PNceKBc` (avatar rotation fix), before that `CA_HLJDfBDvSj8E` (Session AC twice), before that `CA_gHcpYouSmW6Y`, before that `CA_hJH2hG3UdrR3` (Session AA before that: `CA_eESBzjexFe9C`).
